 ...
 *
 * <p>
 * <b>{@link UserResponse} 를 재사용하지 않는 이유</b>: 그쪽은 email, schoolEmail 을 담는
 * "내 프로필" 전용 DTO다. 로그인한 사람이면 누구나 이 API 를 부를 수 있으므로, 그대로 쓰면
 * userId 를 1 부터 훑는 것만으로 전교생 이메일이 수집된다.
 *
 * <p>
 * 그래서 <b>연락처 성격의 값(email, schoolEmail, providerId)은 절대 넣지 않는다</b> —
 ...
 *
 * <p>
 * 슬롯 필드(desiredRoles/skills/experienceLevel/activityStyle)의 집합을 추천 카드와
...
public class UserProfileResponse {
     ...
     *
     * <p>
     * 연락처가 아니라 본인이 보여주려고 쓴 소개글이므로 이 DTO 의 비공개 원칙(위 주석)에
     ...
     *
     * <p>
     * 연락처가 아니므로 이 DTO 의 비공개 원칙(위 주석)에 걸리지 않는다 — 사진은 추천 카드에서도
     * 보여야 하는 공개 항목이다.
     */
     ...
     *
     * <p>
     * 키 이름을 {@code @JsonProperty} 로 못박은 이유: boolean 의 is- 접두사는 Jackson 이
