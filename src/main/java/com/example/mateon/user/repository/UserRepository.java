...
public interface UserRepository extends JpaRepository<User, Long> {
     ...
     *
     * <p>엔티티를 로드해 save 하지 않는 이유: 이 갱신은 비동기 워커(ProfileImageWorker)가 버킷
     * 작업을 끝낸 뒤에 실행되고, 그 사이 사용자가 이름·학과를 고쳤을 수 있다. 워커가 들고 있던
     * 스냅샷을 merge 하면 그 수정이 옛 값으로 되돌아간다.
     *
     * <p>@Transactional 을 메서드에 붙인 이유: 워커는 트랜잭션 없이 돌아야 한다(버킷 호출이
     ...
     *
     * <p>비동기로 버킷 객체를 지우는 동안 사용자가 새 사진을 올렸을 수 있다. 그때 무조건 null 로
