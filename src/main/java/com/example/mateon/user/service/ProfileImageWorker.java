 ...
 *
 * <p>{@link ProfileImageService} 와 나눠 둔 이유는 두 가지다. {@code @Async} 는 프록시를 거쳐야
 ...
 *
 * <p>여기서 나는 실패는 응답으로 전할 길이 없다(사용자는 이미 200 을 받았다). 지금은 로그만
...
public class ProfileImageWorker {
     ...
     *
     * <p>삭제를 먼저 하는 이유: 버킷에 남는 고아 객체를 만들지 않는다. 삭제가 실패하면 업로드도
     * 하지 않고 멈춘다 — 그래야 사용자에게는 이전 사진이 그대로 보이는 일관된 상태로 남고,
     * 지우지 못한 객체가 계속 참조되지 않은 채 쌓이는 일도 없다.
     *
     * <p>예외를 밖으로 던지지 않는다. {@code @Async void} 는 던져도 받는 곳이 없다.
     */
    @Async
     ...
     *
     * <p>URL 을 바로 비우는 이유: 지워진 객체를 가리키는 URL 이 조회 응답에 남아 있으면 프론트가
     ...
     *
     * <p>순서를 이렇게 잡은 이유: DB 를 먼저 비우면 삭제가 실패했을 때 아무도 참조하지 않는 객체가
