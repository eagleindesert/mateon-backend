 ...
 *
 * <p><b>수명 상태는 이 테이블만 갖는다.</b> 도메인 테이블이 자기 status 를 따로 들면 "이 작업이
 ...
 *
 * <p>덕분에 게이트웨이가 도메인을 몰라도 된다 — "이 스레드에 살아 있는 작업이 있나"를 도메인
 * 서비스를 N 번 부르지 않고 여기 한 번의 조회로 답할 수 있다. 그게 이 층을 만든 이유다.
 *
 * <p><b>{@code AuditingEntityListener} 를 일부러 쓰지 않는다.</b> {@code @LastModifiedDate} 는
...
public class AiDomainTask {
     ...
     *
     * <p>이미 닫힌 작업을 다시 닫지 않는다. 완료 직후 재시작 같은 경로에서 종료 사유가
     * 덮어써지면 "왜 끝났는지"가 뒤바뀐다.
     */
