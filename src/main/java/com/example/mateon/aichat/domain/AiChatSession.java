 ...
 *
 * <p><b>상태가 없다.</b> V30 까지는 ACTIVE/CLOSED 와 "사용자당 ACTIVE 1건" 부분 유니크 인덱스로
 * "지금 이어갈 대화"를 하나로 고정했는데, 스레드를 여러 개 갖고 골라 들어가게 되면서 그 개념이
 * 사라졌다 — 어디에 쓸지는 사용자가 요청마다 지정한다.
 *
 * <p><b>도메인 상태도 없다.</b> 진행 상황·만료 판정은 {@link AiDomainTask} 가, 추출 결과처럼
 * 도메인마다 형태가 다른 건 각 도메인 테이블이 갖는다. 여기는 "누가 언제 무슨 말을 했는가"의
 * 그릇일 뿐이라 도메인이 늘어도 컬럼이 변하지 않는다.
 *
 * <p>{@link #lastSeq} 가 채번 카운터인 이유는 두 가지다. 메시지를 붙일 때마다 {@code COUNT(*)}
...
public class AiChatSession {
     ...
     * 다음 seq 를 발급한다.
     *
     * <p>이 호출이 엔티티를 더럽히므로 {@code updatedAt} 도 함께 갱신된다 — 채번과 활동 시각이
