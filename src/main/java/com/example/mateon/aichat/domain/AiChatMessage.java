 ...
 *
 * <p><b>{@link #task} 가 이 설계의 핵심이다.</b> 게이트웨이가 아직 도메인을 못 정했거나(되묻기)
 * 범위 밖이라 직접 답한 턴은 null 로 남는다. 라우팅이 확정되는 순간 그 발화부터 도메인 작업이
 * 찍힌다.
 *
 * <p>덕분에 두 가지가 동시에 성립한다:
 * <ul>
 *   <li>화면 복원은 스레드 전체를 시간순으로 읽으면 된다 — 게이트웨이 턴도 사용자 눈에는 대화다.</li>
 *   <li>도메인 AI 로 보낼 발화는 {@code task} 로 정확히 걸러진다 — 라우팅 전 잡담이 FastAPI 로
 *       새어 나가지 않고, 완료된 옛 작업의 발화가 새 작업에 섞이지도 않는다.</li>
 * </ul>
 *
 * <p>V30 에서는 이 자리가 {@code (domain, domain_ref_id)} 라는 다형 포인터였다. FK 로 바꾸면서
 * 참조 무결성이 생겼고, 매 행 반복되던 domain 값이 작업 행 하나로 모였다.
 *
 * <p>seq 는 스레드 안의 정렬 키일 뿐이다. FastAPI 로 보낼 때는 USER 행만 골라 1..N 으로
...
public class AiChatMessage {
     ...
     *
     * <p>위임할 곳이 없는 판정(UNCLEAR/OUT_OF_SCOPE)은 여기 오지 않는다 — 그런 턴은 task 가
