 ...
 *
 * <p>
 * 지원서 API(TeamController 의 /applications/*)와 방향이 반대인 경로다. URL 도 그 모양을
 * 그대로 뒤집어 두었다: 지원서는 유저가 {teamId}/apply 로 시작하고 팀장이 처리하지만,
 * 제안은 팀장이 {teamId}/offers 로 시작하고 유저가 처리한다.
 *
 * <p>
 * 제안 대상은 GET /api/matching/recommendations/team-to-user 로 찾는다.
 */
...
public class TeamOfferController {
    ...
    public ResponseEntity<BaseResponse<TeamOfferResponseDTO>> createOffer(
    ...
    public ResponseEntity<BaseResponse<List<TeamOfferResponseDTO>>> getTeamOffers(
      @PathVariable Long teamId,
      Authentication authentication
    ) {
    ...
    public ResponseEntity<BaseResponse<List<TeamOfferResponseDTO>>> getMyOffers(
      Authentication authentication
    ) {
        return ResponseEntity.ok(BaseResponse.success(
     ...
     *
     * <p>
     * 이미 응답했거나 팀장이 취소한 제안이면 400 OFFER_ALREADY_RESPONDED,
    ...
    public ResponseEntity<BaseResponse<TeamOfferResponseDTO>> respondToOffer(
    ...
    public ResponseEntity<BaseResponse<String>> cancelOffer(
      @PathVariable Long offerId,
      Authentication authentication
    ) {
        teamOfferService.cancelOffer(offerId, Long.valueOf(authentication.getName()));
        return ResponseEntity.ok(BaseResponse.success("제안이 취소되었습니다."));
    }
}
