...
public interface TeamMemberRepository extends JpaRepository<TeamMember, Long> {
     ...
     *
     * <p>
     * id 만 필요한 호출부용이다 — 프록시에서 getId() 는 초기화 없이 읽히므로 join 이 낭비다.
     ...
     *
     * <p>
     * user 를 함께 읽는다 — 곧바로 이름/전공을 꺼내는 호출부에서 LAZY 로 두면 팀원 수만큼
     * 추가 쿼리가 나간다.
     */
     ...
     *
     * <p>
     * {@link #findActiveMembersWithUser} 와 달리 TeamMember 를 영속성 컨텍스트에 올리지 않는다.
     ...
     *
     * <p>
     * 차이가 하나 있다: 리더도 team_members 에 있으므로 결과가 곧 실제 인원이다.
     * 예전처럼 +1 보정을 하면 안 된다.
     *
     * <p>
     * 멤버가 한 명도 없는 팀은 결과에 나타나지 않는다(GROUP BY 특성). 호출부에서 0 으로 채운다.
     */
