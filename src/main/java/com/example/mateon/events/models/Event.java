...
public class Event {
     ...
     *
     * <p>label 은 화면에 그대로 노출되는 한글 표기다 ({@link Field} 와 같은 구조). 예전에는
     * 이 매핑이 서비스 쪽 switch 문에 문자열로 박혀 있었는데, 표기가 필요한 곳이 늘면서
     * enum 이 자기 표기를 들고 있는 편이 낫다고 판단했다 — 상수를 추가하면 라벨도 강제로
     * 같이 정해지므로 매핑이 빠질 수 없다.
     */
    public enum Category {
        CONTEST("공모전"), EXTERNAL("대외활동"), SCHOOL("교내"), ETC("기타");
