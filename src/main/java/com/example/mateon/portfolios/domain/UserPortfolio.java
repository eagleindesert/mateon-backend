 ...
 *
 * <p>{@link #pdfId} 는 우리가 채번한 값이 아니라 <b>PDF 원본 바이트의 SHA-256</b> 이다.
 * AI 서버가 같은 규약으로 계산해 돌려주지만, 우리도 같은 바이트에서 계산할 수 있으므로
 * AI 를 부르기 전에 이 값으로 캐시를 조회한다 (PortfolioSummaryService 참고).
 *
 * <p>PDF 원본은 저장하지 않는다. 개인 이력이라 공개 버킷에 둘 수 없고, 요약을 이미 갖고 있으면
 * 원본이 다시 필요한 경우가 없다.
 */
@Entity
@Table(
...
public class UserPortfolio {
     ...
     *
     * <p>지금 흐름에서는 캐시 히트면 AI 를 부르지 않으므로 호출되지 않는다. 프롬프트가 바뀌어
