 ...
 *
 * <p>
 * OCI Object Storage 는 <b>버킷 단위 쿼터를 제공하지 않는다</b>. 콤파트먼트 쿼터
 ...
 *
 * <p>
 * <b>단일 인스턴스 전제다</b> (SchedulingConfig 와 같은 제약). 카운터가 JVM 힙에 있으므로
...
public class BucketCapacityGuard {
     ...
     *
     * <p>
     * 1024 단위(GiB)로 나눈다 — {@code storage.max-bytes} 를 파싱하는 Spring 의 DataSize 가
     * 같은 기준이라, 설정에 적은 "2GB" 가 로그에도 "2.00GB" 로 그대로 되비쳐야 한다.
     *
     * <p>
     * Locale.ROOT 를 박는 이유: 기본 로캘이 소수점을 쉼표로 찍는 환경에서 "2,00GB" 가 되면
     * 로그를 기계로 긁을 때 걸린다.
     */
     ...
     *
     * <p>
     * CAS 반복인 이유: 동시에 들어온 업로드 둘이 각자 "아직 여유 있음"을 읽고 둘 다 통과하는
     ...
     *
     * <p>
     * 여기서 통과해도 {@link #reserve} 가 거절할 수 있다(그 사이 다른 업로드가 채웠을 때).
