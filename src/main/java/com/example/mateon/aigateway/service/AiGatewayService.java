 ...
 *
 * <p><b>이 클래스는 도메인의 내부를 모른다.</b> 라우터를 건너뛸지 정할 때 매칭 서비스에 묻지
...
public class AiGatewayService {
     ...
     * 라우터를 부를지 말지부터 정한다.
     *
     * <p>이 스레드에서 진행 중인 작업이 <b>정확히 하나</b>면 분류를 건너뛰고 그리로 통과시킨다.
     * 사용자는 지금 그 AI 의 질문에 답하는 중이라 "백엔드요" 같은 짧은 발화가 이어지는데, 그걸
     * 매 턴 LLM 에 다시 물으면 왕복만 늘고 오분류 위험만 생긴다.
     *
     * <p>둘 이상이면 그러지 않는다 — 어느 쪽에 하는 말인지 모호하니 판단은 LLM 에 맡긴다.
     */
