...
@Schema(description = "AI 채팅 한 턴. 어느 스레드에 쓸지 지정해서 보낸다.")
@Getter
@Setter
@NoArgsConstructor
public class AiGatewayMessageRequestDTO {
    ...
    /**
     * 선택 항목으로 두고 "없으면 새 스레드"로 처리하지 않는다 — 한 엔드포인트가 두 가지 일을
     * 하게 되고, 프론트가 필드를 빠뜨렸을 때 스레드가 조용히 하나 더 생긴다. 스레드 생성은
     * {@code POST /api/ai/chat/sessions} 로 명시적으로 한다.
     */
    @Schema(description = "발화를 붙일 스레드 id. POST /api/ai/chat/sessions 로 받은 값이거나 "
            + "직전 응답의 sessionId.", example = "12")
    @NotNull(message = "스레드 id는 필수입니다.")
