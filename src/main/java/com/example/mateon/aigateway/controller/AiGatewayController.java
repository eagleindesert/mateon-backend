...
/**
 * AI 채팅의 단일 입구. 도메인을 정하고 해당 도메인 대화로 위임한다.
 */
...
public class AiGatewayController {
    ...
    @Operation(summary = "AI 채팅 — 발화 보내고 답변 받기",
            description = """
                          ...
                          * `OUT_OF_SCOPE` — 서비스가 다루지 않는 주제다. `assistantMessage` 를 보여준다.
                          ...
                          판정 없이 무조건 매칭으로 처리되므로 신규 화면은 이쪽을 쓴다.""")
    ...
    public ResponseEntity<BaseResponse<AiGatewayResponseDTO>> submitMessage(
        ...
        AiGatewayResponseDTO response = aiGatewayService.submitMessage(userId, request.getMessage());
