...
public interface MatchingIntentSlotRepository extends JpaRepository<MatchingIntentSlot, Long> {
     ...
     *
     * <p>
     * 슬롯이 있다는 건 곧 의도 추출 완료다. 슬롯과 user_embeddings 는 같은 트랜잭션에서
     * 함께 저장되므로 슬롯만 보면 된다 (벡터는 호출부가 배치로 붙인다).
     *
     * <p>
     * user 를 JOIN FETCH 하는 이유: 후보 최대 200명의 이름/학교를 응답에 실어야 하는데
     ...
     *
     * <p>
     * {@link #findByUserId} 와 달리 user 를 함께 가져오는 이유: 요약 조립의 폴백 마지막 층이
