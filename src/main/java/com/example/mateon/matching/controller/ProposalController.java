 ...
 *
 * <p>
 * <b>여기서는 아무것도 저장하지 않는다.</b> 응답은 초안이고, 사용자가 화면에서 고친 뒤
 ...
 *
 * <p>
 * 선행 조건은 추천 이력이다. 추천에 뜬 적 없는 상대면 404 RECOMMENDATION_NOT_FOUND —
 * 적합도(synergy_score)의 출처가 추천 이력뿐이라 그것 없이는 조립할 수 없다. 추천을 거치지 않고
 * 그냥 지원하는 건 기존 발송 API 로 하면 된다.
 *
 * <p>
 * 조회처럼 보이는데 POST 인 이유: LLM 호출을 유발한다 (상세 이유와 같은 판단).
...
public class ProposalController {
     ...
     *
     * <p>
     * 선행 호출은 GET /api/matching/recommendations/user-to-team 이다.
     */
     ...
     *
     * <p>
     * 선행 호출은 GET /api/matching/recommendations/team-to-user 다.
     *
