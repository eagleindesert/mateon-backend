 ...
 *
 * <p>
 * 선행 조건: 의도 추출(/api/matching/intents)이 완료돼 있어야 한다. 아직이면 400
...
public class RecommendationController {
     ...
     *
     * <p>
     * 추천할 팀이 없으면 빈 배열이다 — "아직 후보가 없음"은 정상 상태라 404 가 아니다.
     *
     ...
     *
     * <p>
     * 여기서 나온 유저에게 POST /api/teams/{teamId}/offers 로 제안을 보내면, 그 유저가
     * 수락 여부를 결정한다 (지원과 승인 주체가 반대인 경로).
     *
     * <p>
     * 후보는 매칭 의도 추출을 마친 유저뿐이다. 이미 이 팀의 팀원/지원자이거나 이 팀에게서
     ...
     *
     * <p>
     * 같은 팀을 다시 요청하면 처음 생성한 문장을 그대로 돌려준다 (AI 재호출 없음). 따라서
     * 문구는 한 번 정해지면 바뀌지 않는다.
     *
     * <p>
     * 추천에 뜬 적 없는 팀이면 404 RECOMMENDATION_NOT_FOUND 다. 먼저
     ...
     *
     * <p>
     * 캐시 규칙과 404 조건은 유저→팀 방향과 같다 (선행 호출은 GET /team-to-user).
     *
