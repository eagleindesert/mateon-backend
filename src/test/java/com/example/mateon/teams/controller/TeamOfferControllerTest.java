 ...
 * 역제안 API 의 전선 계약.
 *
 * <p>이 컨트롤러의 경로들은 <b>같은 {@code /api/teams} 아래에서 이름만으로 갈린다</b>:
 ...
 * 모양으로는 구분되지 않는다.
 *
 * <p>또 하나는 <b>{@code accepted} 가 {@code Boolean} 이라는 것</b>이다. {@code boolean} 이었다면
 * 누락 시 기본값 {@code false}(=거절)로 조용히 처리됐을 텐데, 래퍼 타입 + {@code @NotNull} 이라
 * 400 으로 막힌다. 사용자의 수락 의사를 서버가 임의로 거절로 해석하는 사고를 막는 유일한 장치다.
 *
 * <p>{@code aiScore}/{@code aiLabel} 이 응답에 실려 나가는 것도 확인한다 — 서버가 추천 이력에서
