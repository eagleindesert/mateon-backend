 ...
 * 로컬 전용 카카오 인가코드 수신기.
 *
 * <p>디버그 도구지만 <b>동작이 깨지면 카카오 로그인을 손으로 검증할 방법이 사라진다</b>.
 * 인가코드는 일회용이고 수 분 안에 만료되므로, 셸 스크립트가 읽는 시점에 테이블에는 <b>가장
 * 최근 코드 하나만</b> 있어야 한다.
 *
 * <p>그래서 {@code deleteAllInBatch()} 가 {@code save()} <b>보다 먼저</b>여야 한다. 순서가
 * 뒤집히면 방금 저장한 코드까지 지워져 테이블이 비고, 스크립트는 "코드가 없다"며 실패한다.
 * 반대로 삭제가 빠지면 옛 코드가 남아 스크립트가 만료된 코드를 집어 든다 — 둘 다 브라우저
 * 화면에는 "저장 완료"가 멀쩡히 뜬다.
 *
 * <p>응답이 {@code text/html} 인 것도 확인한다. 브라우저 리다이렉트의 종착지라, JSON 으로
 * 바뀌면 사용자가 원시 JSON 을 보게 된다.
 *
 * <p>{@code @ConditionalOnProperty} 로 기본 미등록이라는 안전장치는 여기서 검증하지 않는다 —
