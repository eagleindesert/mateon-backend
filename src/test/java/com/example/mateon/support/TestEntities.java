 ...
 *
 * <p>이 레포의 관례는 "픽스처는 각 테스트 클래스의 private 헬퍼"다. 그런데 id 만은 예외로
 ...
 *
 * <p>필드명을 문자열로 넘기는 방식이라 이름을 틀리면 실패하는데, 그 실패가 여덟 군데에서
 * 똑같이 재현되는 걸 막으려고 한 곳에 모은다.
 *
 * <p>주의: 서비스가 {@code repository.save(...)} 의 반환값을 그대로 쓰는 경우, 여기서 id 를
