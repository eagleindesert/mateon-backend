# launch.ps1 (for-api-server) - 두 유저(A·B)의 실시간 채팅을 '두 개의 PowerShell 창'으로 띄운다.
# ----------------------------------------------------------------------------
# 사용법:
#   powershell -ExecutionPolicy Bypass -File .\launch.ps1
#
# 사전 조건:
#   - .env 에 MATEON_BASE_URL 을 원격 서버 주소로 지정 (미지정 시 00_common 기본값인 localhost:8080 폴백)
#   - (자동 유저 생성 시) 인증코드를 사람이 직접 입력 — 원격 DB 조회는 닿지 않으므로 로컬 도커 불필요.
...
#      - 먼저 login 시도 → 실패하면 request→(수동 코드)→verify→signup 자동 진행
...
# ----------------------------------------------------------------------------
# 계정 준비: login 우선, 실패 시 request→(수동 코드)→verify→signup 진행
# ----------------------------------------------------------------------------
function Ensure-User {
    ...
    # 원격 서버는 로컬 DB 조회가 닿지 않으므로 Get-EmailVerificationCode 가 콘솔에서 사람이
    # 직접 코드를 입력하도록 안내한다(메일 확인 또는 원격 DB 의 email_verifications.code 조회).
    $code = Get-EmailVerificationCode -Email $Email
    $verificationToken = ""
    if ($code) {
        $verify = Invoke-Api -Method POST -Path "/api/auth/email/verify" -PassThru -Title "인증코드 검증: $Email" -Body @{
...
if (-not $tokenA -or -not $tokenB) {
    Write-Host "`n(!) 계정 준비 실패 — MATEON_BASE_URL 로 서버에 닿는지, 인증코드를 올바르게 입력했는지 확인하세요." -ForegroundColor Red
    return
}
