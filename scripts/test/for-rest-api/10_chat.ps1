# 10_chat.ps1 - Chat (채팅) 테스트  /api/chat + WebSocket(STOMP)  [인증 필요]
...
#   - (자동 유저 B 생성 시) 로컬 PostgreSQL 도커 컨테이너 실행 중 (이메일 인증코드 DB 조회)
#
# 흐름:
#   1) 유저 A(현재 토큰) + 유저 B(2번째 계정, 자동 생성/로그인) 준비
...
# --- 유저 B 준비: signup(이메일 인증 자동) 시도 후 실패하면 login ---
Write-Host "`n[10.0 유저 B 준비] $UserBEmail" -ForegroundColor Cyan
Invoke-Api -Method POST -Path "/api/auth/email/request" -Title "10.0a B 이메일 인증코드 요청" -Body @{ email = $UserBEmail } | Out-Null
$codeB = Get-EmailVerificationCode -Email $UserBEmail
if ($codeB) {
    Invoke-Api -Method POST -Path "/api/auth/email/verify" -Title "10.0b B 이메일 인증코드 검증" -Body @{
        email = $UserBEmail; code = $codeB
    } | Out-Null
}
$tokenB = $null
if ($codeB) {
    $signupB = Invoke-Api -Method POST -Path "/api/auth/signup" -PassThru -Title "10.0c B 회원가입" -Body @{
        email = $UserBEmail; password = $UserBPassword; passwordConfirm = $UserBPassword
        name = $UserBName; campus = "JUKJEON"; college = "SW융합대학"; major = "소프트웨어학과"; grade = "3학년"
    }
    if ($signupB.data.accessToken) { $tokenB = $signupB.data.accessToken }
}
if (-not $tokenB) {
    # 이미 가입된 계정이면 로그인으로 토큰 확보
    $loginB = Invoke-Api -Method POST -Path "/api/auth/login" -PassThru -Title "10.0d B 로그인" -Body @{
        email = $UserBEmail; password = $UserBPassword
    }
    if ($loginB.data.accessToken) { $tokenB = $loginB.data.accessToken }
}
if (-not $tokenB) {
    Write-Host "(!) 유저 B 토큰 확보 실패 - DB 컨테이너/계정 상태를 확인하세요. (채팅 테스트 중단)" -ForegroundColor Red
    return
}
...
# 이후 REST 는 다시 A 토큰으로 (B signup/login 이 파일을 건드리지 않았지만 확실히 복구)
Use-Token $tokenA
