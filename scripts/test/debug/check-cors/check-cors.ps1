...
#
# 1) REST API CORS: SecurityConfig.corsConfigurationSource() 는 debug.enabled 값에 따라
...
#   - 아예 없으면 → 그 오리진은 차단된 것
#
# 2) WebSocket(STOMP) CORS: WebSocketConfig.registerStompEndpoints() 도 동일한
...
param(
    [string]$Path    = "/health",    # REST preflight 대상 경로 (permitAll 인 헬스체크가 기본값)
    [string]$WsPath  = "/ws-stomp",  # WebSocket 핸드셰이크 대상 경로
...
Write-Host "`n########## WebSocket(STOMP) 핸드셰이크 CORS 확인 ##########" -ForegroundColor Magenta
...
if ($wsOtherPassed.Count -gt 0) {
    Write-Host "  결론: WebSocket CORS 가 현재 '풀려' 있습니다 (미등록 오리진도 통과 -> debug.enabled=true 추정)" -ForegroundColor Yellow
} else {
    $wsKnownBlocked = @($wsResults | Where-Object { $_.Known -and $_.Blocked })
    if ($wsKnownBlocked.Count -gt 0) {
        Write-Host "  결론: 정식 등록된 오리진 중 일부가 WebSocket 에서 차단되고 있습니다. WebSocketConfig 설정을 확인하세요." -ForegroundColor Red
    } else {
        Write-Host "  결론: WebSocket CORS 가 정상적으로 제한되어 있습니다 (등록된 오리진만 통과, debug.enabled=false 추정)" -ForegroundColor Green
    }
}
Write-Host ("=" * 70) -ForegroundColor DarkGray
